...
\tkzDefPointBy[projection = onto C--B](A) \tkzGetPoint{HA}
\tkzDefPointBy[projection = onto A--D](B) \tkzGetPoint{HB}
\tkzDefPointBy[projection = onto A--D](C) \tkzGetPoint{HC}
\tkzDefPointBy[projection = onto C--B](D) \tkzGetPoint{HD}
...
\tkzDrawSegments[line width=0.75pt,color=red](A,HA B,HB C,HC D,HD)

\tkzDrawSegments[line width=0.5pt,color=black,dashed,dash pattern=on 1pt off 1.75pt,add=0 and 0.75](C,HD A,HB)
...
\tkzLabelPoint[above](HB){$\rm H_B$}
\tkzLabelPoint[above](HC){$\rm H_\Gamma$}
\tkzLabelPoint[below](HD){$\rm H_\Delta$}
