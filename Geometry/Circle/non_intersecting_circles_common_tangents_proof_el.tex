...
\tkzDefPointBy[translation=from B to O2](A) \tkzGetPoint{E}

\tkzDrawCircle[line width=1.0pt,fill=white](O1,A)
\tkzDrawCircle[line width=1.0pt,fill=white](O2,B)

\tkzMarkRightAngles[line width=0.7pt, size=.15,color=AngleClr,fill=AngleClr,fill opacity=0.2](O1,A,B O2,B,A O1,E,O2) %  O1,C,D O2,D,C)
...
% \tkzDefPointBy[translation=from A to O1](B) \tkzGetPoint{Z}
...
\tkzDrawSegments[line width=0.75pt,color=black,dashed,dash pattern=on 1pt off 1.75pt](O1,E E,O2 O2,B O1,O2) %  O1,C O2,D)

\tkzDrawPoints[size=3](O1,O2) %,TA,TB,TC,TD)
\tkzDrawPoints[size=2](A,B,E)
\tkzLabelPoint[left](O1){$\rm O_1$}
\tkzLabelPoint[right](O2){$\rm O_2$}
\tkzLabelPoint[above](A){$\rm A$}
\tkzLabelPoint[below](B){$\rm B$}
\tkzLabelPoint[above](E){$\rm O_2'$}
% \tkzLabelPoint[below](C){$\rm \Gamma$}
% \tkzLabelPoint[above](D){$\rm \Delta$}

\tkzLabelSegment[scale=0.8,above left](O1,A){$r_1$}
\tkzLabelSegment[scale=0.8,above left](A,E){$r_2$}
\tkzLabelSegment[scale=0.8,below right](B,O2){$r_2$}
\tkzLabelSegment[scale=0.8,below right](O1,O2){$\delta$}
