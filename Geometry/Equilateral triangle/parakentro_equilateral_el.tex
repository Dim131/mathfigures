...
\tkzDefPoints{0/0/A,1.95/3.38/C,3.9/0/B}
...
\tkzFillAngles[fill=AngleClr,size=.3,fill opacity=0.1](IC,A,B H2,A,IC)
\tkzMarkAngles[line width=1pt,size=.3,color=AngleClr,mark=|,mksize=2](IC,A,B H2,A,IC)

\tkzMarkRightAngles[line width=1pt, size=.15,color=AngleClr,fill=AngleClr,fill opacity=0.1](IC,H,B IC,H2,A IC,H3,A)


\tkzDrawSegment[line width=0.5pt,color=black,dashed,dash pattern=on 1pt off 1.75pt,add=0.75 and 0](B,C)
\tkzDrawSegment[line width=0.5pt,color=black,dashed,dash pattern=on 1pt off 1.75pt,add=.75 and 0](A,C)
...
\tkzDrawPoints[size=3](A,B,C,IC,H3)

\tkzLabelPoint[left](A){$\rm \Gamma$}
\tkzLabelPoint[right](B){$\rm B$}
\tkzLabelPoint[above](C){$\rm A$}
\tkzLabelPoint[below](IC){$\rm J_A$}
\tkzLabelPoint[below right](H3){$\rm \Delta$}
